...
Подробнее см. в [{#T}](datamodel/backup-collection.md).
...
**Полная резервная копия** — это полный снимок всех данных в коллекции. Служит основой для [цепочек резервных копий](#backup-chain) и может быть восстановлен независимо.

#### Инкрементальная резервная копия {#incremental-backup}

**Инкрементальная резервная копия** — это копирование только изменений (вставки, обновления, удаления), произошедших с момента предыдущего резервной копии. Для восстановления требуется вся цепочка резервных копий.
