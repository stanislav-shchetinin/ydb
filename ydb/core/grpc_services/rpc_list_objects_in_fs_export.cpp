...
using TEvListObjectsInFsExportRequest = TGrpcRequestOperationCall<Ydb::Import::ListObjectsInFsExportRequest,
    Ydb::Import::ListObjectsInFsExportResponse>;

class TListObjectsInFsExportRPC: public TRpcOperationRequestActor<TListObjectsInFsExportRPC, TEvListObjectsInFsExportRequest> {
public:
    using TBase = TRpcOperationRequestActor<TListObjectsInFsExportRPC, TEvListObjectsInFsExportRequest>;
    using TRpcOperationRequestActor<TListObjectsInFsExportRPC, TEvListObjectsInFsExportRequest>::TRpcOperationRequestActor;

    explicit TListObjectsInFsExportRPC(IRequestOpCtx* request)
        : TBase(request)
    ...
    STATEFN(StateFunc) {
        switch (ev->GetTypeRewrite()) {
            hFunc(NKikimr::NSchemeShard::TEvImport::TEvListObjectsInFsExportResponse, Handle);
    ...
    void Bootstrap() {
        ...
        const auto& settings = GetProtoRequest()->settings();
        if (!TFsPath(settings.base_path()).IsAbsolute()) {
            return Reply(Ydb::StatusIds::BAD_REQUEST, "base_path must be an absolute path", NKikimrIssues::TIssuesIds::YDB_API_VALIDATION_ERROR, NActors::TActivationContext::AsActorContext());
        }

        TString error;
        if (!ValidateFsPath(settings.base_path(), "base_path", error)) {
            return Reply(Ydb::StatusIds::BAD_REQUEST, error, NKikimrIssues::TIssuesIds::YDB_API_VALIDATION_ERROR, NActors::TActivationContext::AsActorContext());
        }

        ResolveDatabase();

        Become(&TListObjectsInFsExportRPC::StateFunc);
    }

    void ResolveDatabase() {
        YDB_LOG_DEBUG("[ListObjectsInFsExport] Resolve database",
            {"selfId", SelfId()},
    ...
    void Handle(TEvTxProxySchemeCache::TEvNavigateKeySetResult::TPtr& ev) {
        ...
        YDB_LOG_DEBUG("[ListObjectsInFsExport] Handle TEvTxProxySchemeCache::TEvNavigateKeySetResult",
            {"selfId", SelfId()},
        ...
        if (!domainInfo) {
            YDB_LOG_ERROR("[ListObjectsInFsExport] Got empty domain info",
                {"selfId", SelfId()});
    ...
    void SendRequestToSchemeShard() {
        YDB_LOG_DEBUG("[ListObjectsInFsExport] Send request",
            {"selfId", SelfId()},
        ...
        auto request = MakeHolder<NSchemeShard::TEvImport::TEvListObjectsInFsExportRequest>();
        ...
        *request->Record.MutableSettings() = GetProtoRequest()->settings();
        request->Record.MutableSettings()->set_base_path(StripTrailingSlashes(request->Record.GetSettings().base_path()));
    ...
    void Handle(NKikimr::NSchemeShard::TEvImport::TEvListObjectsInFsExportResponse::TPtr& ev) {
        const auto& record = ev->Get()->Record;

        YDB_LOG_DEBUG("[ListObjectsInFsExport] Handle TListObjectsInFsExportRPC::TEvListObjectsInFsExportResponse",
            {"selfId", SelfId()},
    ...
    void DeliveryProblem() {
        YDB_LOG_WARN("[ListObjectsInFsExport] Delivery problem",
            {"selfId", SelfId()});
...
void DoListObjectsInFsExportRequest(std::unique_ptr<IRequestOpCtx> p, const IFacilityProvider& f) {
    f.RegisterActor(new TListObjectsInFsExportRPC(p.release()));
}
