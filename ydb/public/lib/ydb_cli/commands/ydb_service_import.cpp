...
template <>
void TCommandImportBase::FillItemsFromItemParam(NImport::TImportFromFsSettings& settings) const {
    ApplyItems(settings);
}

template <>
void TCommandImportBase::FillItemsFromItemParam(NImport::TImportFromS3Settings& settings) const {
#if defined(_win32_)
    ApplyItems(settings);
#else
    InitAwsAPI();
