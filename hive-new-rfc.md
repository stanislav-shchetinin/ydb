...
1. Создание backup-таблиц и метаданных таблеток (`PrepareCopyTable`): добавить `TEvCreateTablet` для bulk create пачками, группируя таблетки по `ObjectId`, чтобы уменьшить поток сообщений между SchemeShard и Hive.
...
6. Удаление backup-таблиц после выгрузки: использовать bulk delete (`TEvDeleteTablet`) пачками, уменьшая количество запросов и ответов (аналогично п1).
...
- Для bulk-delete используется существующий `TEvDeleteTablet` со списком шардов (с указанием `TabletId` или `OwnerId` -- но с последним могут быть трудности); в SchemeShard добавляется обработка списочных ответов. `OK` означает принятие удаления; backup delete queue отдельно ограничивает delete storage.
...
- **`PrepareCopyTable` (база SchemeShard):** новый сохраняемый жизненный цикл предсозданного набора backup-таблеток: ожидает готовности после запуска, подготовлен и ожидает `CopyTable`, передан в `CopyTable`.
...
- **Backup boot/delete и окно snapshot:** размер очереди, возраст старейшей записи (сколько времени самая давно ожидающая операция находится в очереди), занятые слоты (разрешение выполнять одну операцию одновременно с другими), фактический темп.
